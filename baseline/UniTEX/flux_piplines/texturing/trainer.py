    ...
    def log_validation(
        ...
        if os.path.isfile(args.validation_prompt):
            ...
            for validation_prompt_item in tqdm(validation_prompt, total=len(validation_prompt), desc='log_validation'):
                ...
                if args.control_image:
                    if args.both_ccm_normal_condition:
                        image1 = validation_prompt_item['control_image']['normal']
                        image2 = validation_prompt_item['control_image']['ccm']
                        ...
                        image = validation_prompt_item['control_image']['normal']
                ...
                if accelerator.is_main_process:
                    ...
                    images[1].save(args.output_dir+f'/eval_result_step_{epoch}/{eval_count}'+f'/control_image.png')
                    images[2].save(args.output_dir+f'/eval_result_step_{epoch}/{eval_count}'+f'/gen_image.png')
                eval_count += 1
            ...
            with autocast_ctx:
                ...
                images[1].save(args.output_dir+f'/eval_result_step_{epoch}'+f'/control_image_{eval_count}.png')
                images[2].save(args.output_dir+f'/eval_result_step_{epoch}'+f'/gen_image_{eval_count}.png')
                eval_count += 1
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                with accelerator.accumulate(models_to_accumulate):
                    ...
                    #     batch["albedos"],
                    #     batch["roughnesses"].repeat(1,3,1,1),
                    #     batch["metallics"].repeat(1,3,1,1),
                    # ], dim=2).to(device=accelerator.device)
                    #breakpoint()
                    pixel_values = batch["training_image"].to(device=accelerator.device)
                    # pixel_values = torch.cat([
                    #     batch["training_image"],
                    #     torch.cat([batch["metallics"],batch["roughnesses"],torch.ones_like(batch["metallics"])],dim=1),
                    #     batch["normals"],
                    # ], dim=2).to(device=accelerator.device)
                    # if self.args.multiscale_training:
                    #     if self.args.multiscale_training_step_range[0] <= global_step <= self.args.multiscale_training_step_range[1]:
                    #         idx = random.choices(range(len(self.args.multiscale_training_probabilities)), self.args.multiscale_training_probabilities)[0]
                    #         # NOTE: ValueError: align_corners option can only be set with the interpolating modes: linear | bilinear | bicubic | trilinear
                    #         pixel_values = torch.nn.functional.interpolate(
                    #             pixel_values,
                    #             size=(self.args.multiscale_training_heights[idx], self.args.multiscale_training_widths[idx]), 
                    #             mode='nearest', 
                    #             align_corners=None,
                    #         )
                    ...
                    if self.args.control_image:
                        if self.args.both_ccm_normal_condition:
                            control_image = 0.5*batch["native_normals"] + 0.5*batch["ccms"]
                        else:
                            control_image = batch["native_normals"]
