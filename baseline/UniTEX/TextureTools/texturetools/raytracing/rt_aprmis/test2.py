...
#let's have a test
m_intersect_test = slangtorch.loadModule('bvhworkers/intersect_test2.slang')
# generating rays
y, x = torch.meshgrid([torch.linspace(1, -1, 1024), 
                       torch.linspace(-1, 1, 1024)], indexing='ij')
z = -torch.ones_like(x)
...
hit_tid_map = torch.zeros((num_rays, 1), dtype=torch.int32, device='cuda')
...
m_intersect_test.intersect(num_rays=int(num_rays), rays_o=ray_origins, rays_d=ray_directions,
               ...
               hit_map=hit, hit_tid_map=hit_tid_map, hit_pos_map=hit_pos_map, hit_uv_map=hit_uv_map)\
...
#locs = hit.repeat(1,3)
locs = hit_pos_map
uvs = torch.cat([hit_uv_map, 1.0 - hit_uv_map.sum(dim=-1, keepdim=True)], dim=-1)
pyexr.write(f'./color_xyz.exr', locs.reshape(1024,1024,3).cpu().numpy())
pyexr.write(f'./color_uv.exr', uvs.reshape(1024,1024,3).cpu().numpy())