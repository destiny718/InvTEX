'''
Mesh refine in MVLRM/AIDoll/iris3d_cr/MVMeshRecon/MVDiffusion
* vertex color
* UV + texture
'''
...
from ...mesh.structure import Texture, Mesh
from .uv_dilation import uv_dilation
...
def refine_mesh_vertex(
    mesh: Mesh, 
    cam2world_matrices: torch.Tensor, 
    intrinsics: torch.Tensor, 
    input_rgb: torch.Tensor, 
    v_attr: Optional[torch.Tensor]=None,
    ...
    if v_attr is None:
        v_attr = torch.zeros_like(mesh.v_pos)
        confidence = 1.0
    ...
    for iter in range(n_iters):
        ...
        cur_ret_dict = mesh_renderer.simple_rendering(
            mesh, v_attr, None, None,
...
def refine_mesh_uv(
    ...
    use_dilation=True, dilation_every_iter=False,
    use_grad_mask=True, grad_mask_antimoire=False,
    ...
    if map_Kd is None:
        ...
        map_Kd = torch.zeros((texture_size, texture_size, 3), dtype=cam2world_matrices.dtype, device=cam2world_matrices.device)
        use_soft_blending = False
        confidence = 1.0
    if map_Kd.shape[-1] == 3:
        map_Kd = torch.cat([map_Kd, mesh.compute_uv_mask(texture_size=map_Kd.shape[0]).to(dtype=map_Kd.dtype, device=map_Kd.device)], dim=-1)
    ...
    map_Kd_mask = (map_Kd_mask > 0.6).to(dtype=map_Kd_mask.dtype)  # > 0.5 slightly for blured mask
    map_Kd = map_Kd[..., :3]
    ...
    for iter in range(n_iters):
        ...
        cur_ret_dict = mesh_renderer.simple_rendering(
            mesh, v_weight, map_Kd_input, None,
            c2w_mat, intr_mat, H_dst, 
            render_v_attr=use_vertex_normal,
            render_uv=True,
            render_map_attr=True,
            background=None,
            grid_interpolate_mode="nearest", 
        ...
        loss = sum(v for v in loss_dict.values())
        loss.backward()
        if use_grad_mask and iter == 0:
            map_Kd_grad_mask = ((map_Kd.grad.data.abs() > 0).sum(-1, keepdim=True) > 0)
            if grad_mask_antimoire:
                map_Kd_grad_mask = map_Kd_grad_mask.to(dtype=torch.float32).permute(2, 0, 1).unsqueeze(0)
                ...
                map_Kd_grad_mask = map_Kd_grad_mask.squeeze(0).permute(1, 2, 0).to(dtype=torch.bool)
        # map_Kd.grad = map_Kd.grad.masked_fill(~map_Kd_grad_mask, 0.0)
        optimizer.step()
        optimizer.zero_grad(set_to_none=True)
        
        if visualize:
            cv2.imwrite(f'./.cache/map_Kd_{iter:04d}.png', map_Kd.detach().flip(-1).cpu().numpy() * 255)
    ...
    with torch.no_grad():
        ...
        if use_dilation:
            ...
            if use_grad_mask:
                _map_Kd_mask = map_Kd_grad_mask.permute(2, 0, 1).unsqueeze(0)
            else:
                _map_Kd_mask = map_Kd_mask.permute(2, 0, 1).unsqueeze(0)
            map_Kd = uv_dilation(map_Kd, ~_map_Kd_mask)
        ...
        if use_soft_blending:
            map_Kd_fin = image_fusion(map_Kd_input.unsqueeze(0), map_Kd_init.unsqueeze(0), map_Kd_grad_mask.unsqueeze(0)).squeeze(0)
        else:
