# Expected metadata columns (CSV):
# - prompt: text prompt
# - video: target RGB video (output)
# - vace_video: depth video control (1-channel 8-bit is OK)
# - vace_normal_video: normal video control (3-channel, [0,255])
# - vace_video_mask: mask video control (1-channel 8-bit)
# Note: `num_frames` should satisfy `num_frames % 4 == 1` for Wan video models.

accelerate launch examples/wanvideo/model_training/train.py \
  --dataset_base_path data/example_video_dataset \
  --dataset_metadata_path data/example_video_dataset/metadata_vace_depth_normal.csv \
